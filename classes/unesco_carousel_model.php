<?php
    ...
    Class Carousel{
            ...
            if(isset($_POST['carousel_title'])) {
                if(isset($_FILES['fileupload']) && $_FILES['fileupload']['error'] === UPLOAD_ERR_OK) {
                    ...
                    if (move_uploaded_file($tempname, $folder)) {
                        $insert_stmt=$this->db->connect()->prepare("INSERT INTO unesco_carousel (carousel_title, carousel_content, filename) VALUES (:carousel_title, :carousel_content, :filename)");
        ...
        public function fetchAllRecords() {
			...
			$select_stmt = $this->db->connect()->prepare('SELECT id, carousel_title, carousel_content, filename FROM unesco_carousel;');
			$select_stmt->execute();
        ...
        public function deleteRecords($delete_id) {
            $delete_stmt = $this->db->connect()->prepare('DELETE FROM unesco_carousel WHERE id = :sid ');
        ...
        public function edit($update_id) {
            ...
            $edit_stmt = $this->db->connect()->prepare('SELECT * FROM unesco_carousel WHERE id = :sid');
            ...
            if(isset($_POST['edit_title']) || isset($_POST['edit_content']) || isset($_POST['edit_id'])) {
                if(!empty($_POST['edit_title']) || !empty($_POST['edit_content']) || !empty($_POST['edit_id'])) {
                    ...
                    if(isset($_FILES['fileupload']) && $_FILES['fileupload']['error'] === UPLOAD_ERR_OK) {
                        ...
                        if(move_uploaded_file($tempname, $folder)) {
                            $update_stmt=$this->db->connect()->prepare('UPDATE unesco_carousel SET carousel_title=:carousel_title, carousel_content=:carousel_content, filename=:filename WHERE id=:id');
                        ...
                        $update_stmt=$this->db->connect()->prepare('UPDATE unesco_carousel SET carousel_title=:carousel_title, carousel_content=:carousel_content WHERE id=:id');
						$update_stmt->bindParam(':carousel_title', $carousel_title);
						$update_stmt->bindParam(':carousel_content', $carousel_content);
						$update_stmt->bindParam(':id', $id);
