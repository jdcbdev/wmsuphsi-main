<?php
    ...
    Class News{
            ...
            if(isset($_POST['news_title'])) {
                if(isset($_FILES['fileupload']) && $_FILES['fileupload']['error'] === UPLOAD_ERR_OK) {
                    ...
                    if (move_uploaded_file($tempname, $folder)) {
                        $insert_stmt=$this->db->connect()->prepare("INSERT INTO unesco_news (news_title, image_description, news_content, filename) VALUES (:news_title, :image_description, :news_content, :filename)");
        ...
        public function fetchAllRecords() {
			...
			$select_stmt = $this->db->connect()->prepare('SELECT id, news_title, image_description, news_content, created_at, updated_at, filename FROM unesco_news;');
			$select_stmt->execute();
        ...
        public function deleteRecords($delete_id) {
            $delete_stmt = $this->db->connect()->prepare('DELETE FROM unesco_news WHERE id = :sid ');
        ...
        public function edit($update_id) {
            ...
            $edit_stmt = $this->db->connect()->prepare('SELECT * FROM unesco_news WHERE id = :sid');
            ...
            if(isset($_POST['edit_title']) && isset($_POST['edit_description']) && isset($_POST['edit_content']) && isset($_POST['edit_id'])) {
                if(!empty($_POST['edit_title']) && !empty($_POST['edit_description']) && !empty($_POST['edit_content']) && !empty($_POST['edit_id'])) {
                    ...
                    if(isset($_FILES['fileupload']) && $_FILES['fileupload']['error'] === UPLOAD_ERR_OK) {
                        ...
                        if(move_uploaded_file($tempname, $folder)) {
                            $update_stmt=$this->db->connect()->prepare('UPDATE unesco_news SET news_title=:news_title, image_description=:image_description, news_content=:news_content, filename=:filename WHERE id=:id');
                        ...
                        $update_stmt=$this->db->connect()->prepare('UPDATE unesco_news SET news_title=:news_title, image_description=:image_description, news_content=:news_content WHERE id=:id');
						$update_stmt->bindParam(':news_title', $news_title);
						$update_stmt->bindParam(':image_description', $image_description);
                        $update_stmt->bindParam(':news_content', $news_content);
						$update_stmt->bindParam(':id', $id);
