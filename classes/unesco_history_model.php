<?php
    ...
    Class History{
            ...
            if(isset($_POST['history_title'])) {
                if(isset($_FILES['fileupload']) && $_FILES['fileupload']['error'] === UPLOAD_ERR_OK) {
                    ...
                    if (move_uploaded_file($tempname, $folder)) {
                        $insert_stmt=$this->db->connect()->prepare("INSERT INTO unesco_history (history_title, history_description, filename) VALUES (:history_title, :history_description, :filename)");
        ...
        public function fetchAllRecords() {
			...
			$select_stmt = $this->db->connect()->prepare('SELECT id, history_title, history_description, filename FROM unesco_history;');
			$select_stmt->execute();
        ...
        public function deleteRecords($delete_id) {
            $delete_stmt = $this->db->connect()->prepare('DELETE FROM unesco_history WHERE id = :sid ');
        ...
        public function edit($update_id) {
            ...
            $edit_stmt = $this->db->connect()->prepare('SELECT * FROM unesco_history WHERE id = :sid');
            ...
            if(isset($_POST['edit_title']) && isset($_POST['edit_description']) && isset($_POST['edit_id'])) {
                if(!empty($_POST['edit_title']) && !empty($_POST['edit_description']) && !empty($_POST['edit_id'])) {
                    ...
                    if(isset($_FILES['fileupload']) && $_FILES['fileupload']['error'] === UPLOAD_ERR_OK) {
                        ...
                        if(move_uploaded_file($tempname, $folder)) {
                            $update_stmt=$this->db->connect()->prepare('UPDATE unesco_history SET history_title=:history_title, history_description=:history_description, filename=:filename WHERE id=:id');
                        ...
                        $update_stmt=$this->db->connect()->prepare('UPDATE unesco_history SET history_title=:history_title, history_description=:history_description WHERE id=:id');
						$update_stmt->bindParam(':history_title', $history_title);
						$update_stmt->bindParam(':history_description', $history_description);
						$update_stmt->bindParam(':id', $id);
