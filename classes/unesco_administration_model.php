<?php
    ...
    Class Administration{
            ...
            if(isset($_POST['admin_name'])) {
                if(isset($_FILES['fileupload']) && $_FILES['fileupload']['error'] === UPLOAD_ERR_OK) {
                    ...
                    if (move_uploaded_file($tempname, $folder)) {
                        $insert_stmt=$this->db->connect()->prepare("INSERT INTO unesco_administration (admin_name, admin_position, filename) VALUES (:admin_name, :admin_position, :filename)");
        ...
        public function fetchAllRecords() {
			...
			$select_stmt = $this->db->connect()->prepare('SELECT id, admin_name, admin_position, filename FROM unesco_administration;');
			$select_stmt->execute();
        ...
        public function deleteRecords($delete_id) {
            $delete_stmt = $this->db->connect()->prepare('DELETE FROM unesco_administration WHERE id = :sid ');
        ...
        public function edit($update_id) {
            ...
            $edit_stmt = $this->db->connect()->prepare('SELECT * FROM unesco_administration WHERE id = :sid');
        ...
        public function update($edit_id) {
            ...
            if(isset($_POST['edit_name']) && isset($_POST['edit_position']) && isset($_POST['edit_id'])) {
                if(!empty($_POST['edit_name']) && !empty($_POST['edit_position']) && !empty($_POST['edit_id'])) {
                    ...
                    if(isset($_FILES['fileupload']) && $_FILES['fileupload']['error'] === UPLOAD_ERR_OK) {
                        ...
                        if(move_uploaded_file($tempname, $folder)) {
                            $update_stmt=$this->db->connect()->prepare('UPDATE unesco_administration SET admin_name=:admin_name, admin_position=:admin_position filename=:filename WHERE id=:id');
                        ...
                        $update_stmt=$this->db->connect()->prepare('UPDATE unesco_administration SET admin_name=:admin_name, admin_description=:admin_description WHERE id=:id');
						$update_stmt->bindParam(':admin_name', $admin_name);
                        $update_stmt->bindParam(':admin_position', $admin_position);
						$update_stmt->bindParam(':id', $id);
